...
//   async fn new<P>(iota_client: IotaClient, signer: impl Signer<IotaKeySignature>) ->
// anyhow::Result<NotarizationBuilder<S>> {     let builder = NotarizationBuilder::new(iota_client).await?
//       .signer(signer);
//     Ok(builder)
//   }

//   async fn new_with_pkg_id(iota_client: IotaClient, signer: impl Signer<IotaKeySignature>, iota_product_pkg_id:
// ObjectID) -> anyhow::Result<Self::FullProduct> {     let builder = NotarizationBuilder::new_with_pkg_id(iota_client,
// iota_product_pkg_id).await?       .signer(signer);
//     Ok(builder)
//   }
...
//   async fn new_read_only_with_pkg_id(iota_client: IotaClient, iota_product_pkg_id: ObjectID) ->
// anyhow::Result<Self::ReadOnlyProduct> {     Ok(NotarizationBuilder::new_read_only_with_pkg_id(iota_client,
// iota_product_pkg_id).await?)   }
