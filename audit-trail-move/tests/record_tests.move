...
#[test]
fun test_delete_tagged_record_with_matching_role_tags() {
    let admin = @0xAD;
    let mut scenario = ts::begin(admin);
    ...
    ts::next_tx(&mut scenario, admin);
    {
        ...
        trail.create_role(
            &admin_cap,
            string::utf8(b"TaggedRecordAdmin"),
            permission::record_admin_permissions(),
            std::option::some(record_tags::new_role_tags(vector[string::utf8(b"finance")])),
            &clock,
            ts::ctx(&mut scenario),
...
fun test_delete_tagged_record_requires_matching_role_tags() {
    ...
    ts::next_tx(&mut scenario, admin);
    {
        ...
        trail
            ...
            .create_role(
                &admin_cap,
                string::utf8(b"DeleteOnly"),
                permission::from_vec(vector[permission::delete_record()]),
                std::option::none(),
                &clock,
                ts::ctx(&mut scenario),
            );

        let tagged_record_cap = test_utils::new_capability_without_restrictions(
            trail.access_mut(),
            &admin_cap,
            &string::utf8(b"TaggedRecordAdmin"),
            &clock,
        ...
        transfer::public_transfer(tagged_record_cap, admin);
    ...
    ts::next_tx(&mut scenario, admin);
    {
        ...
        let tagged_record_cap = ts::take_from_sender<Capability>(&scenario);
        ...
        tagged_record_cap.destroy_for_testing();
        cleanup_capability_trail_and_clock(&scenario, delete_only_cap, trail, clock);
    };
    ...
    ts::next_tx(&mut scenario, admin);
    {
        ...
        trail.delete_record(&delete_only_cap, 0, &clock, ts::ctx(&mut scenario));
