...
fun test_count_based_locking() {
    ...
    {
        let locking_config = locking::new(locking::window_count_based(2), timelock::none(), timelock::none());
        let (admin_cap, _) = setup_test_audit_trail(
            &mut scenario,
            locking_config,
            std::option::none(),
...
fun test_count_based_locking_single_record() {
    ...
    {
        let locking_config = locking::new(locking::window_count_based(3), timelock::none(), timelock::none());
        let (admin_cap, _) = setup_test_audit_trail(
            &mut scenario,
            locking_config,
...
fun test_delete_record_after_time_lock_expires() {
    ...
    {
        let locking_config = locking::new(locking::window_time_based(3600), timelock::none(), timelock::none()); // 1 hour = 3600 seconds
        let (admin_cap, _) = setup_test_audit_trail(
            &mut scenario,
            locking_config,
...
fun test_delete_records_batch_bypasses_record_lock() {
    ...
    // Create trail with 1 hour delete lock and an initial record.
    {
        ...
        let (admin_cap, _) = setup_test_audit_trail(
            &mut scenario,
            locking_config,
            std::option::some(new_test_data(1, b"Locked")),
        );
        transfer::public_transfer(admin_cap, admin);
    };
...
fun test_delete_records_batch_requires_delete_all_records_permission() {
...

    {
        ...
        let (admin_cap, _) = setup_test_audit_trail(
            &mut scenario,
            locking_config,
            std::option::some(new_test_data(1, b"Record")),
        );
        transfer::public_transfer(admin_cap, admin);
    };
