...
use audit_trail::main::{AuditTrail};
use audit_trail::test_utils::{Self, TestData, setup_test_audit_trail, fetch_capability_trail_and_clock, cleanup_capability_trail_and_clock};
...
fun setup_trail_with_record_admin_capability_and_time_window_restriction(
    ...
    ts::next_tx(scenario, admin_user);
    {
        ...
        let cap = trail.roles_mut().new_capability(
            &admin_cap,
...
/// Returns the trail_id.
fun setup_trail_with_record_admin_role(
    scenario: &mut Scenario,
    admin_user: address,
): ID {
...
fun test_new_capability() {
    ...
    let cap1_id = {
        ...
        let cap1 = trail.roles_mut().new_capability_without_restrictions(
            &admin_cap,
            &string::utf8(b"RecordAdmin"),
            &clock,
            ts::ctx(&mut scenario),
    ...
    let _cap2_id = {
        ...
        let cap2 = trail.roles_mut().new_capability_without_restrictions(
            &admin_cap,
            &string::utf8(b"RecordAdmin"),
            &clock,
            ts::ctx(&mut scenario),
...
fun test_revoke_capability() {
    ...
    ts::next_tx(&mut scenario, admin_user);
    {
        ...
        trail.roles_mut().revoke_capability(
            &admin_cap,
            cap1.id(),
            &clock,
            ts::ctx(&mut scenario),
...
fun test_capability_lifecycle() {
    ...
    let (record_cap_id, role_cap_id) = {
        ...
        let record_cap = trail.roles_mut().new_capability_without_restrictions(
            &admin_cap,
            &string::utf8(b"RecordAdmin"),
            &clock,
            ts::ctx(&mut scenario),
        ...
        let role_cap = trail.roles_mut().new_capability_without_restrictions(
            &admin_cap,
            &string::utf8(b"RoleAdmin"),
            &clock,
            ts::ctx(&mut scenario),
...
/// Test capability with only issued_to restriction (address-restricted capability).
///
/// This test validates:
/// - Capability can only be used by the specified address
/// - Capability is not restricted by time
/// - Other users cannot use the capability even if they possess it
...
fun test_capability_issued_to_only() {
    ...
    ts::next_tx(&mut scenario, admin_user);
    {
        ...
        let cap = trail.roles_mut().new_capability_for_address(
            &admin_cap,
            &string::utf8(b"RecordAdmin"),
            authorized_user,
...
fun test_capability_valid_until_only() {
    ...
    ts::next_tx(&mut scenario, admin_user);
    {
        ...
        let cap = trail.roles_mut().new_capability_valid_until(
            &admin_cap,
            &string::utf8(b"RecordAdmin"),
...
fun test_is_valid_for_timestamp() {
    ...
    ts::next_tx(&mut scenario, admin_user);
    {
        ...
        let unrestricted_cap = trail.roles_mut().new_capability_without_restrictions(
            &admin_cap,
            &string::utf8(b"RecordAdmin"),
            &clock,
            ts::ctx(&mut scenario),
...
fun test_new_capability_without_restrictions() {
    ...
    ts::next_tx(&mut scenario, admin_user);
    {
        ...
        let cap = trail.roles_mut().new_capability_without_restrictions(
            &admin_cap,
            &string::utf8(b"RecordAdmin"),
            &clock,
            ts::ctx(&mut scenario),
