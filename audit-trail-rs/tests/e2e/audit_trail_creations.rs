...
use audit_trails::core::types::{Capability, Data, ImmutableMetadata, LockingConfig};
...
async fn create_trail_with_default_builder_settings() -> anyhow::Result<()> {
    ...
    println!("Created trail with ID: {}", created.trail_id);
...
async fn create_trail_with_custom_admin_address() -> anyhow::Result<()> {
    // let _guard = e2e_test_guard().await;
    let client = get_funded_test_client().await?;
    let custom_admin = IotaAddress::from_str("0x1111111111111111111111111111111111111111111111111111111111111111")?;
    ...
    let cap = get_cap(&client, custom_admin, created.trail_id).await;
...
pub(crate) async fn get_cap<C>(client: &C, owner: IotaAddress, trail_id: ObjectID) -> anyhow::Result<ObjectRef>
where
