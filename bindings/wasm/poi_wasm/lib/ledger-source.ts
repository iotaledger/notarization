...
import { createIotaGrpcClient, type IotaGrpcClient, type IotaGrpcClientOptions } from "./client.js";
...
export class LedgerSource implements LedgerSourceContract {
    ...
    public async transaction(
        ...
        for await (
            const response of this.#client.getTransactions({
                requests: {
    ...
    public async object(
        ...
        for await (
            const response of this.#client.getObjects({
                requests: {
                    requests: [
                        {
                            objectRef: {
    ...
    public async checkpoint(
        ...
        for await (
            const response of this.#client.getCheckpoint({
                checkpointId: {
                    case: "sequenceNumber",
