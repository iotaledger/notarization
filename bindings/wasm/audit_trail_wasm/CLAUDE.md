# CLAUDE.md — Documentation Style Guide for `audit_trail_wasm`

This file is the authoritative style guide for the doc comments on the wasm-bindgen
bindings in `bindings/wasm/audit_trail_wasm/src/`. The Rust doc comments here are
shipped to TypeScript consumers verbatim — they end up in the generated `.d.ts`
files and drive IDE IntelliSense and TypeDoc output. They must therefore read as
TSDoc/JSDoc, **not** as rustdoc.

## Audience

The reader is a TypeScript developer using the audit-trail SDK. They:
...
```
/// Requires the {@link Permission.AddRoles} and {@link Permission.AddCapabilities}
/// permissions.
```
...
- Logical preconditions that the SDK rejects before submission.
...
Public fields of `getter_with_clone` structs are visible to TS as plain
...
2. `cargo doc --target wasm32-unknown-unknown --no-deps` — should produce no
   `rustdoc::all` warnings (the crate has `#![warn(rustdoc::all)]`).
